...
MG_INTERNAL void mg_call(struct mg_connection *nc, ///////////////////call
                         mg_event_handler_t ev_handler, int ev, void *ev_data) {
  if (ev_handler == NULL) {
    /*
...
void mg_if_connect_cb(struct mg_connection *nc, int err) { /////////////////call MG_EV_CONNECT
...
#endif
  {
    n = (int) MG_SEND_FUNC(nc->sock, io->buf, io->len, 0); ///////////////////////////////send to the data
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now) {//////////////////call mg_mgr_handle_conn
  int worth_logging =
  ...
  if (!(nc->flags & MG_F_CLOSE_IMMEDIATELY)) {
    if ((fd_flags & _MG_F_FD_CAN_WRITE) && nc->send_mbuf.len > 0) {
      mg_write_to_socket(nc);/////////////////////////write to socket
    }
...
time_t mg_socket_if_poll(struct mg_iface *iface, int timeout_ms) {
  ...
  for (nc = mgr->active_connections; nc != NULL; nc = tmp) {///////////////////MG_F_CLOSE_IMMEDIATELY
    tmp = nc->next;
    if ((nc->flags & MG_F_CLOSE_IMMEDIATELY) ||
